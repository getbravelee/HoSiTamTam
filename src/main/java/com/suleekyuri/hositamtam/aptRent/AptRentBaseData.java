...
@Configuration
@Profile({"dev"})
@ConditionalOnProperty(name = "data.loading.rent.enabled", havingValue = "true")
public class AptRentBaseData {

    @Value("${public.data.key}")
    private String apiKey;

    @Bean
    CommandLineRunner initAptRentData(AptRentService aptRentService) {
        List<Integer> list = lawDongCodeSet();
        List<Integer> monthList = monthList(202301, 202411);
        return args -> {
            aptRentService.callApiAndSaveData(apiKey, list, monthList);
    ...
    private List<Integer> lawDongCodeSet() {
        ...
        for(int n : AptRentBaseData.lawDongCodes) {
